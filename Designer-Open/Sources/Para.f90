...
module Para
    ...
    logical :: para_write_303   = .true.        ! *Modified geometry seperated from vertex, ModGeo_Chimera_Mod_Geometry,      "_sep_geo.bild"
    logical :: para_write_401   = .true.       !  Cross-sectional geometry,                Section_Chimera_Cross_Geometry,   "_cro_geo.bild"
    ...
    logical :: para_chimera_303_info = .true.  !  Plot with edge and point number (_sep_geo.bild)
    logical :: para_chimera_401_info = .true.  !  Plot with edge and point number (_cro_geo.bild)
    logical :: para_chimera_502_ori  = .true.  !  Plot with helix z-direction (_line.bild / _node.bild)
    logical :: para_chimera_503_mod  = .true.  !  Plot with modified edges (_mesh.bild)
    logical :: para_chimera_504_info = .true.  !  Plot with edge and point number (_cross_geo_mod.bild)
    logical :: para_chimera_601_dir  = .true.  !  Plot with strand direction (_scaf.bild / _stap.bild)
    logical :: para_chimera_609_cyl  = .true.  !  Plot with cylinderical representation (_atom.bild)
    logical :: para_chimera_609_dir  = .true.  !  Plot with strand direction (_atom.bild)
contains
