...
subroutine Route_Generation(prob, geom, mesh, dna)
    ...
    write(p_redir, "(a)")
    write(p_redir, "(a)"), " +==================================================+"
    write(p_redir, "(a)"), " | 5. Scaffold routing                              |"
    write(p_redir, "(a)"), " +==================================================+"
    write(p_redir, "(a)")
    ...
    call Route_Reconnect_Junction(geom, mesh, dna)
    ...
    call Route_Chimera_Crossovers(prob, geom, mesh, dna)
...
subroutine Route_Init_Base_Connectivity(mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a)"), "  5.1. Initialize DNA data"
    write(p_redir, "(a)"), "   * # of initial nts in the scaffold : "//trim(adjustl(Int2Str(dna.n_base_scaf)))
    write(p_redir, "(a)"), "   * # of initial nts in stapples     : "//trim(adjustl(Int2Str(dna.n_base_stap)))
    write(p_redir, "(a)")
...
subroutine Route_Set_Base_Position(geom, mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a)"), "  5.2. Set the position vector for nucleotides"
    write(p_redir, "(a)"), "   * Edge section type        : "//trim(geom.sec.types)//" lattice"
    write(p_redir, "(a)"), "   * Starting basepair ID     : "//trim(adjustl(Int2Str(para_start_bp_ID)))
    write(p_redir, "(a)"), "   * Axial rise distance [nm] : "//trim(adjustl(Dble2Str(para_dist_bp)))
    write(p_redir, "(a)"), "   * Radius of helix [nm]     : "//trim(adjustl(Dble2Str(para_rad_helix)))
    write(p_redir, "(a)"), "   * The gap between helixes  : "//trim(adjustl(Dble2Str(para_gap_helix)))
    write(p_redir, "(a)"), "   * Angle of minor groove    : "//trim(adjustl(Dble2Str(para_ang_minor)))
    write(p_redir, "(a)"), "   * Angle correction factor  : "//trim(adjustl(Dble2Str(para_ang_correct)))
    ...
    do i = 1, mesh.n_node
        ...
        if(p_detail == .true.) then
            write(p_redir, "(i10$)"), mesh.node(i).id
            write(p_redir, "(a, a3$)"), " node ==> bp: ", trim(adjustl(Int2Str(mesh.node(i).bp)))
            write(p_redir, "(a, a5$)"), ", up: ",   trim(adjustl(Int2Str(mesh.node(i).up)))
            write(p_redir, "(a, a5$)"), ", dn: ",   trim(adjustl(Int2Str(mesh.node(i).dn)))
            write(p_redir, "(a, a2$)"), ", sec: ",  trim(adjustl(Int2Str(mesh.node(i).sec)))
            write(p_redir, "(a, a3$)"), ", iniL: ", trim(adjustl(Int2Str(mesh.node(i).iniL)))
            write(p_redir, "(a, a3$)"), ", croL: ", trim(adjustl(Int2Str(mesh.node(i).croL)))
            write(p_redir, "(a, a5$)"), ", start ang: ", trim(adjustl(Dble2Str(ang_start)))
            write(p_redir, "(a, a5$)"), ", scaf ang: ",  trim(adjustl(Dble2Str(ang_scaf)))
            write(p_redir, "(a, a5 )"), ", stap ang: ",  trim(adjustl(Dble2Str(ang_stap)))
        end if
    end do
...
subroutine Route_Chimera_Route(prob, geom, mesh, dna, step_route)
    ...
    if(step_route == "route3" .and. para_tecplot == .true.) then
    ...
    ! ---------------------------------------------
    if(step_route /= "route3" .or. para_tecplot /= .true.) return
...
! Connect strands at the vertex by poly Tn loop or unpaired nucleotides
subroutine Route_Reconnect_Junction(geom, mesh, dna)
    type(GeomType), intent(inout) :: geom
    type(MeshType), intent(in)    :: mesh
    type(DNAType),  intent(inout) :: dna
    ...
    ! Print progress
    write(p_redir, "(a)"), "  5.3. Edge connection at the vertex"
    ...
    write(p_redir, "(a)"), "   * # of rows on the edge section : "//trim(adjustl(Int2Str(geom.sec.maxR-geom.sec.minR+1)))
    write(p_redir, "(a)"), "   * # of cols on the edge section : "//trim(adjustl(Int2Str(geom.sec.maxC-geom.sec.minC+1)))
    write(p_redir, "(a)"), "   * Detailed info. on the connection"
    ...
    do i = 1, geom.n_junc
        ...
        allocate(conn(geom.n_sec*geom.junc(i).n_arm, 2))

        conn(:, 1) = geom.junc(i).conn(:, 1)
        conn(:, 2) = geom.junc(i).conn(:, 2)

        ! Print progress
        if(p_detail == .true.) then
            write(p_redir, "(i10$)"), i
            write(p_redir, "(a$  )"), " junc -> # of nodes connected :"
            write(p_redir, "(i3  )"), geom.n_sec*geom.junc(i).n_arm

            do j = 1, geom.n_sec*geom.junc(i).n_arm
                write(p_redir, "(i20, a$)"), conn(j, 1), " node -->"
                write(p_redir, "(i6,  a )"), conn(j, 2), " node"
            end do
        end if
        ...
        ! --------------------------------------------------
        ! Allocate join
        allocate(join(geom.n_sec*geom.junc(i).n_arm/2, 2))
        ...
        ! --------------------------------------------------
        do j = 1, geom.n_sec * geom.junc(i).n_arm / 2
            ...
            if(geom.sec.conn(mesh.node(node_cur).sec + 1) == -1) then
                ...
                geom.junc(i).n_un_scaf = geom.junc(i).n_un_scaf + n_un_scaf
                geom.junc(i).n_un_stap = geom.junc(i).n_un_stap + n_un_stap
            end if
        end do
    ...
    write(p_redir, "(a)"), "   * # of unpaired nts in scaffold : "//trim(adjustl(Int2Str(n_add_nt_scaf)))
    write(p_redir, "(a)"), "   * # of unpaired nts in staples  : "//trim(adjustl(Int2Str(n_add_nt_stap)))
    write(p_redir, "(a)"), "   * Unparied nts in scaf and stap : "
    write(p_redir, "(a)"), "        th junc    scaf      stap   scaf + stap"
    write(p_redir, "(a)"), "        -------    ----      ----   -----------"
    do j = 1, geom.n_junc
        write(p_redir, "(4i11)"), j, geom.junc(j).n_un_scaf, geom.junc(j).n_un_stap, &
            geom.junc(j).n_un_scaf + geom.junc(j).n_un_stap
    end do
    ...
    if(p_detail == .true.) then
        ...
        do j = 1, dna.n_base_scaf
            write(p_redir, "(i10$  )"), dna.base_scaf(j).id
            write(p_redir, "(a, i5$)"), " scaf ==> node: ", dna.base_scaf(j).node
            if(dna.base_scaf(j).node == -1) then
                write(p_redir, "(a, i5$)"), ", bp: ", -1
            else
                write(p_redir, "(a, i5$)"), ", bp: ", mesh.node(dna.base_scaf(j).node).bp
            end if
            write(p_redir, "(a, i5$)"), ", up: ", dna.base_scaf(j).up
            write(p_redir, "(a, i5$)"), ", dn: ", dna.base_scaf(j).dn
            write(p_redir, "(a, i5$)"), ", xo: ", dna.base_scaf(j).xover
            write(p_redir, "(a, i5 )"), ", ac: ", dna.base_scaf(j).across
        end do

        ! Staple
        write(p_redir, "(a)")
        do j = 1, dna.n_base_stap
            write(p_redir, "(i10$  )"), dna.base_stap(j).id
            write(p_redir, "(a, i5$)"), " stap ==> node: ", dna.base_stap(j).node
            if(dna.base_stap(j).node == -1) then
                write(p_redir, "(a, i5$)"), ", bp: ", -1
            else
                write(p_redir, "(a, i5$)"), ", bp: ", mesh.node(dna.base_stap(j).node).bp
            end if
            write(p_redir, "(a, i5$)"), ", up: ", dna.base_stap(j).up
            write(p_redir, "(a, i5$)"), ", dn: ", dna.base_stap(j).dn
            write(p_redir, "(a, i5$)"), ", xo: ", dna.base_stap(j).xover
            write(p_redir, "(a, i5 )"), ", ac: ", dna.base_stap(j).across
        end do
    end if
    write(p_redir, "(a)")
...
function Route_Connect_Scaf(mesh, dna, node_cur, node_com) result(count)
    ...
    if(dna.base_scaf(cur).up == -1 .and. dna.base_scaf(com).dn == -1) then
        ...
        write(p_redir, "(a)"), " | Wrong connection ID                              |"
        write(p_redir, "(a)"), " +==================================================+"
        stop
    end if
...
subroutine Route_Set_Strand_ID_Scaf(dna)
    ...
    ! Print information
    write(p_redir, "(a)"), "  5.4. Set the strand ID on nts of the scaffold"
    write(p_redir, "(a)"), "   * Set the strand ID in dna.base_scaf data"
    ...
    do i = 1, dna.n_base_scaf
        ...
        do
            ...
            if(count > dna.n_base_scaf) then
                write(p_redir, "(a)")
                write(p_redir, "(a)"), " +=== error ========================================+"
                write(p_redir, "(a)"), " | dna.n_base_scaf exceeds count                    |"
                write(p_redir, "(a)"), " +==================================================+"
                stop
            end if
        end do
    ...
    ! Print progress
    write(p_redir, "(a)"), "   * # of segmented scaffolds : "//trim(adjustl(Int2Str(dna.n_scaf)))
    write(p_redir, "(a)")
...
subroutine Route_Find_Centered_Scaf_Xover(prob, geom, mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a)"), "  5.5. Find centered scaffold crossovers"
    write(p_redir, "(a)"), "   * The number of multiple edges   : "//trim(adjustl(Int2Str(geom.n_croL)))
    write(p_redir, "(a)"), "   * The number of sections         : "//trim(adjustl(Int2Str(geom.n_sec)))
    ...
    do i = 1, mesh.n_node
        ...
        do j = i + 1, mesh.n_node
            ...
            if((pre_iniL == 0 .or. iniL /= pre_iniL) .and. sec_cur == 0) then
                ...
                if((prob.sel_edge_sec /= 1) .and. (sec_cur /= 0 .or. sec_com /= 5)) cycle
    ...
    ! Print progress
    write(p_redir, "(a)"), "   * # of centered scaffold Xovers  : "//trim(adjustl(Int2Str(dna.n_xover_scaf)))
    write(p_redir, "(a)")
...
subroutine Route_Modify_Scaf_Xover(geom, mesh, dna)
    ...
    ! Print information
    write(p_redir, "(a)"), "  5.6. Modify centered crossover to avoid"
    ...
    write(p_redir, "(a)"), "   * # of deleted duplicate indexing Xovers : "&
        //trim(adjustl(Int2Str(n_xover2 - dna.n_xover_scaf)))
    write(p_redir, "(a)"), "   * # of scaffold Xovers                   : "&
...
subroutine Route_Graph_Build_Origami(prob, mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a)"), "  5.7. Scaffold routing"
    write(p_redir, "(a)"), "   * Build dual-graph"
    write(p_redir, "(a)"), "     - # of nodes in the dual-graph : "//trim(adjustl(Int2Str(n_node)))
    write(p_redir, "(a)"), "     - # of edges in the dual-graph : "//trim(adjustl(Int2Str(n_edge)))
    write(p_redir, "(a)"), "   * Generate the spanning tree"
    ...
    if(para_all_spanning == "on" .and. n_edge <= 12) then
        ...
        write(p_redir, "(a$)"), "     - of of all spanning trees : "
        write(p_redir, "(i7)"), SpanTree_Count_Spanning_Trees(adj)
    ...
    write(p_redir, "(a)"), "   * Find the minimum spanning tree"
    write(p_redir, "(a)"), "     - The toal cost for this path : "//trim(adjustl(Int2Str(length)))
    write(p_redir, "(a)"), "     - The minimum spanning tree"
    write(p_redir, "(a)"), "        Arc    Tail node  Head node  Cost"
    write(p_redir, "(a)"), "        ---    ---------  ---------  ----"
    do j = 1, size(tree)
        write(p_redir, "(4i10)"), tree(j), tail(tree(j)), head(tree(j)), cost(tree(j))
...
subroutine Route_Graph_Chimera_All_Spanning_Tree(prob, pos_node, tail, head, idx, src, dst)
    ...
    logical :: results, f_axis, b_branch
    ...
    n_edge  = size(tail)
    results = systemqq("mkdir "//trim(prob.path_work)//"/stree/")

    do k = 1, n_tree

        ! File open
        path = trim(prob.path_work)//"/stree/"//trim(prob.name_file)
...
subroutine Route_Make_Scaf_Origami(prob, mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a )"), "  5.12. Continuous scaffold strand"
    write(p_redir, "(a$)"), "   * # of deleted possible Xovers  : "
    write(p_redir, "(i7)"), n_xover
    write(p_redir, "(a$)"), "   * # of possible scaffold Xovers : "
    write(p_redir, "(i7)"), dna.n_xover_scaf
    write(p_redir, "(a )"), "   * Detailed info. on continuous scaffold"
    ...
    do
        if(p_detail == .true.) then
            if(mod(count, 100) == 0) then
                write(p_redir, "(i7 , a )"), up,    "->"
            else if(mod(count, 100) == 1) then
                write(p_redir, "(i20, a$)"), count, " base - "
                write(p_redir, "(i7 , a$)"), up,    "->"
            else
                write(p_redir, "(i7 , a$)"), up,    "->"
            end if
        ...
        if(count > dna.n_base_scaf) then
            write(p_redir, "(a)")
            write(p_redir, "(a)"), " +=== error ========================================+"
            write(p_redir, "(a)"), " | Not single circular scaffold.                    |"
            write(p_redir, "(a)"), " +==================================================+"
            stop
        end if
    end do
    ...
    if(count /= dna.n_base_scaf) then
        write(p_redir, "(a)")
        write(p_redir, "(a)"), " +=== error ========================================+"
        write(p_redir, "(a)"), " | Not visited bases or strands.                    |"
        write(p_redir, "(a)"), " +==================================================+"
        stop
    end if
...
subroutine Route_Find_Possible_Stap_Xover(geom, mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a)"), "  5.8. Set possible staple crossovers"
    ...
    ! Print progress
    write(p_redir, "(a)"), "   * # of possible staple Xovers : "//trim(adjustl(Int2Str(dna.n_xover_stap)))
    write(p_redir, "(a)")
...
subroutine Route_Set_Stap_Crossover(prob, dna)
    ...
    ! Print information
    write(p_redir, "(a)"), "  5.12. Set staple crossovers"
    ...
    do i = 1, dna.n_base_stap
        ...
        if(cur_xover /= -1 .and. (up /= cur_xover .or. down /= cur_xover)) then
            if(up_xover /= -1) then
                ...
                write(p_redir, "(a)")
                write(p_redir, "(a)"), " +=== error ========================================+"
                write(p_redir, "(a)"), " | The number of crossover should be even.          |"
                write(p_redir, "(a)"), " +==================================================+"
                stop
            end if
        end if
...
! Write crossovers based on base pairs
subroutine Route_Chimera_Crossovers(prob, geom, mesh, dna)
    type(ProbType), intent(in) :: prob
    type(GeomType), intent(in) :: geom
    type(MeshType), intent(in) :: mesh
    type(DNAType),  intent(in) :: dna
    ...
    do i = 1, geom.n_junc
        do j = 1, geom.n_sec*geom.junc(i).n_arm
            ...
            if( mesh.node(geom.junc(i).conn(j, 1)).conn == 2 .or. &
                mesh.node(geom.junc(i).conn(j, 1)).conn == 4 ) then
                write(607, "(a)"), ".color red"
            else if( mesh.node(geom.junc(i).conn(j, 1)).conn == 1 .or. &
                     mesh.node(geom.junc(i).conn(j, 1)).conn == 3 ) then
                write(607, "(a)"), ".color blue"
            end if

            pos_1(:) = mesh.node(geom.junc(i).conn(j, 1)).pos
            pos_2(:) = mesh.node(geom.junc(i).conn(j, 2)).pos
...
subroutine Route_Delete_End_Pos_Xover_Scaf(prob, geom, mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a )"), "  5.8. Deleting possible scaffod Xovers"
    write(p_redir, "(a$)"), "   * Parameter for deleting Xovers near junction : "
    write(p_redir, "(i7)"), para_avoid_gap_scaf
    ...
    ! Print progress
    write(p_redir, "(a)"), "* # of original possible Xovers : "&
        //trim(adjustl(Int2Str(n_xover_scaf_pre)))
    write(p_redir, "(a)"), "* # of deleted possible Xovers  : "&
        //trim(adjustl(Int2Str(n_xover_scaf_pre - dna.n_xover_scaf)))
    write(p_redir, "(a)"), "* # of current possible Xovers  : "&
        //trim(adjustl(Int2Str(dna.n_xover_scaf)))
...
subroutine Route_Delete_End_Pos_Xover_Stap(prob, geom, mesh, dna)
    ...
    ! Print progress
    write(p_redir, "(a )"), "  5.9. Deleting possible crossovers in staple strand"
    write(p_redir, "(a)"), "   * Parameter for deleting xovers near junction : "&
    ...
    ! Print progress
    write(p_redir, "(a$)"), "* # of original possible Xovers : "
    write(p_redir, "(a )"), trim(adjustl(Int2Str(n_xover_stap_pre)))
    write(p_redir, "(a$)"), "* # of deleted possible Xovers  : "
    write(p_redir, "(a )"), trim(adjustl(Int2Str(n_xover_stap_pre - dna.n_xover_stap)))
    write(p_redir, "(a$)"), "* # of current staple Xovers    : "
    write(p_redir, "(a )"), trim(adjustl(Int2Str(dna.n_xover_stap)))
    write(p_redir, "(a$)"), "* # of single staple Xovers     : "
    write(p_redir, "(a )"), trim(adjustl(Int2Str(dna.n_sxover_stap)))
...
subroutine Route_Center_Scaf_Crossover(geom, mesh, dna)
    ...
    ! Print information
    write(p_redir, "(a )"), "  5.11. Delete boundary crossovers in scaffolds"
    write(p_redir, "(a$)"), "   * # of original Xovers : "
    write(p_redir, "(a )"), trim(adjustl(Int2Str(dna.n_xover_scaf)))
