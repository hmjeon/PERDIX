...
! Edge section generation
subroutine Section_Generation(prob, geom)
    ...
    write(p_redir, "(a)")
    write(p_redir, "(a)"), " +==================================================+"
    write(p_redir, "(a)"), " | 3. Build edge section                            |"
    write(p_redir, "(a)"), " +==================================================+"
    write(p_redir, "(a)")

    ! Set sectional data, croP and croL
    call Section_Set_Sectional_Data(geom)
...
function Section_Connection_Scaf(geom, sec_cur, sec_com, bp_id) result(b_connect)
    ...
    !write(p_redir, "(i5, a, 2i)"), sec_cur, " : section(current),   row and col # : ", row_cur, col_cur
    !write(p_redir, "(i5, a, 2i)"), sec_com, " : section(comparing), row and col # : ", row_com, col_com
    !write(p_redir, "(a)")
...
subroutine Section_Set_Sectional_Data(geom)
    type(GeomType), intent(inout) :: geom
    ...
    do i = 1, geom.n_sec
        ...
        do j = 1, geom.n_modP
            ...
            do m = 1, geom.n_junc
                do n = 1, geom.junc(m).n_arm

                    ! if junction data is equal to modified point number, j
                    if(geom.junc(m).modP(n) == j) then
                        geom.junc(m).croP(n, i) = geom.n_modP*(i-1)+j
                    end if
                end do
            end do
        end do
    ...
    ! print progress
    write(p_redir, "(a)"), "  3.1. Edge section information"
    ...
    write(p_redir, "(a)"), "   * Reference min/max column       : "//trim(adjustl(Int2Str(geom.sec.ref_minC)))//" / "//trim(adjustl(Int2Str(geom.sec.ref_maxC)))
    write(p_redir, "(a)"), "   * The number of modified points  : "//trim(adjustl(Int2Str(geom.n_modP)))
    write(p_redir, "(a)"), "   * The number of modified edges   : "//trim(adjustl(Int2Str(geom.n_iniL)))
    write(p_redir, "(a)"), "   * The number of sectional points : "//trim(adjustl(Int2Str(geom.n_croP)))
    write(p_redir, "(a)"), "   * The number of sectional edges  : "//trim(adjustl(Int2Str(geom.n_croL)))
    write(p_redir, "(a)")
    write(p_redir, "(a)"), "  3.2. Update junctional data"
    ...
    if(p_detail == .true.) then
        do m = 1, geom.n_junc
            ...
            write(p_redir, "(i5$)"), m
            write(p_redir, "(a$ )"), " - junc, # arms :"
            write(p_redir, "(i4 )"), geom.junc(m).n_arm

            do n = 1, geom.junc(m).n_arm

                write(p_redir, "(a18$)"), "Mod point #:"
                write(p_redir, "(i4$ )"), geom.junc(m).modP(n)
                write(p_redir, "(a$  )"), " -> cross point # : ("

                do i = 1, geom.n_sec - 1
                    write(p_redir, "(i4, a$)"), geom.junc(m).croP(n, i), ","
                end do

                write(p_redir, "(i4, a)"), geom.junc(m).croP(n, geom.n_sec), " )"
            end do
        end do
    end if
...
subroutine Section_Generate_Section_Geometry(geom)
    ...
    ! Print progress
    write(p_redir, "(a)"), "  3.3. Generate edge section"
    write(p_redir, "(a)")
    write(p_redir, "(a)"), "  3.4. Update local vectors"
    write(p_redir, "(a)")
