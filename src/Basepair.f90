...
subroutine Basepair_Discretize(prob, geom, mesh)
    ...
    write(p_redir, "(a)")
    write(p_redir, "(a)"), " +==================================================+"
    write(p_redir, "(a)"), " | 4. Build the basepairs                           |"
    write(p_redir, "(a)"), " +==================================================+"
    write(p_redir, "(a)")
    ...
    call Basepair_Generate_Basepair(geom, mesh)

    ! Set sectional connection at the junction
    call Basepair_Set_Conn_Junction(geom, mesh)

    ! Write the prior cylindrial model with the orientation
    call Basepair_Chimera_Cylinder_Ori(prob, geom, mesh, "cylinder_prior")

    ! Write the prior cylindrial model
    call Basepair_Chimera_Cylinder(prob, geom, mesh, "cylinder_prior")

    ! Modify the length of the duplex at the junction
    call Basepair_Modify_Junction(prob, geom, mesh)

    ! Delete ghost node from node data
    call Basepair_Delete_Ghost_Node(geom, mesh)

    ! Add one base to 5'-end
    call Basepair_Make_Sticky_End(geom, mesh)

    ! Write the final cylindrial model with the orientation
    call Basepair_Chimera_Cylinder_Ori(prob, geom, mesh, "cylinder_final")

    ! Write the final cylindrial model
    call Basepair_Chimera_Cylinder(prob, geom, mesh, "cylinder_final")
...
subroutine Basepair_Count_Basepair(prob, geom, mesh)
    ...
    do i = 1, geom.n_croL
        ...
        if( (geom.sec.types == "square"    .and. prob.n_edge_len - 1 > count ) .or. &
            (geom.sec.types == "honeycomb" .and. prob.n_edge_len - 2 > count .and. prob.sel_edge_ref == 0) ) then
            write(p_redir, "(a)")
            write(p_redir, "(a)"), " +=== warning ======================================+"
            write(p_redir, "(a)"), " | Not equal number of basepair.                    |"
            write(p_redir, "(a)"), " +==================================================+"
        end if
    end do

    ! Print progress
    write(p_redir, "(a)"), "  4.1. Pre-calculate the number of basepairs"
    write(p_redir, "(a)"), "   * The number of multiple points : "//trim(adjustl(Int2Str(geom.n_croP)))
    write(p_redir, "(a)"), "   * The number of multiple lines  : "//trim(adjustl(Int2Str(geom.n_croL)))
    write(p_redir, "(a)"), "   * The number of basepairs       : "//trim(adjustl(Int2Str(mesh.n_node)))
    write(p_redir, "(a)"), "   * The number of connections     : "//trim(adjustl(Int2Str(mesh.n_ele)))
    write(p_redir, "(a)")
...
! Generate the basepair model
subroutine Basepair_Generate_Basepair(geom, mesh)
    type(GeomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    ...
    ! Print progress
    write(p_redir, "(a)"), "  4.2. Discretize multiple lines into basepairs"
    ...
    do i = 1, geom.n_croL
        ...
        do j = 1, geom.n_junc
            do k = 1, geom.junc(j).n_arm
                do m = 1, geom.n_sec

                    ! # of geom.n_modP is equal to geom.n_croP / geom.n_croP
                    !poi_arm = geom.junc(j).modP(k) + (m - 1) * geom.n_croP / geom.n_sec
                    poi_arm = geom.junc(j).croP(k, m)

                    if(poi_1 == poi_arm) geom.junc(j).node(k, m) = node_start
                    if(poi_2 == poi_arm) geom.junc(j).node(k, m) = n_node
                end do
            end do
        end do
        ...
        if(p_detail == .true.) then
            write(p_redir, "(i5$   )"), i
            write(p_redir, "(a, i3$)"), " line (",            n_node-node_start + 1
            write(p_redir, "(a, i5$)"), "-bp) - start node:", node_start
            write(p_redir, "(a, i5$)"), ", end node:",        n_node
            write(p_redir, "(a, i3$)"), ", sec:",             geom.croL(i).sec
            write(p_redir, "(a, i3 )"), ", init edge:",       geom.croL(i).iniL
        end if
    end do
    write(p_redir, "(a)")
...
! Set sectional connection at the junction
subroutine Basepair_Set_Conn_Junction(geom, mesh)
    type(GeomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    ...
    ! Print progress
    write(p_redir, "(a)"), "  4.3. Set the connection at the vertex"
    write(p_redir, "(a)"), "   * The number of vertices          : "//trim(adjustl(Int2Str(geom.n_junc)))
    write(p_redir, "(a)"), "   * The number of duplexes per edge : "//trim(adjustl(Int2Str(geom.n_sec)))
    write(p_redir, "(a)")
    ...
    do i = 1, geom.n_junc
        ...
        allocate(connect(geom.junc(i).n_arm*geom.n_sec, 2))

        ! all nodes in the junction
        do j = 1, geom.n_sec
            do k = 1, geom.junc(i).n_arm

                ! Find the current node and its initial edge
                node_cur = geom.junc(i).node(k, j)
                ...
                if(nei_line(1) == -1 .and. nei_line(2) == -1) then
                    ...
                    connect((j-1)*geom.junc(i).n_arm+k, 1) = node_cur
                    connect((j-1)*geom.junc(i).n_arm+k, 2) = -1
                    cycle
                end if
                ...
                do m = 1, geom.n_sec
                    do n = 1, geom.junc(i).n_arm
                        node_com = geom.junc(i).node(n, m)
                ...
                do m = 1, geom.n_sec
                    ...
                    if(row_cur == row_com) then
                        ...
                        if(dir_cur == dir_com .and. col_cur == Geom.sec.maxC - col_com + 1) then
                            connect((j-1)*geom.junc(i).n_arm+k, 1) = node_cur
                            connect((j-1)*geom.junc(i).n_arm+k, 2) = node_con(m)
        ...
        do j = 1, geom.n_sec*geom.junc(i).n_arm
            ...
            geom.junc(i).conn(j, 2) = connect(j, 2)
    ...
    do i = 1, geom.n_junc
        do j = 1, geom.n_sec*geom.junc(i).n_arm
            if(geom.junc(i).conn(j, 2) == -1) then
                do k = j + 1, geom.n_sec*geom.junc(i).n_arm
                    if(geom.junc(i).conn(k, 2) == -1) then

                        ! --------------------------------------------------
                        ! For the connection of the multilayered cross-section
                        ! geom.junc(i).conn(j,k), (j: node #, k: node # to be connected))
                        sec_cur = mesh.node(geom.junc(i).conn(j, 1)).sec
                        sec_com = mesh.node(geom.junc(i).conn(k, 1)).sec
                        row_cur = Geom.sec.posR(sec_cur + 1)
                        row_com = Geom.sec.posR(sec_com + 1)
                        col_cur = Geom.sec.posC(sec_cur + 1)
                        col_com = Geom.sec.posC(sec_com + 1)

                        if(row_cur == row_com .and. col_cur == col_com) then
                            geom.junc(i).conn(j, 2) = geom.junc(i).conn(k, 1)
                            geom.junc(i).conn(k, 2) = geom.junc(i).conn(j, 1)
                        end if
                        ! --------------------------------------------------
                    end if
                end do
        ...
        do j = 1, geom.junc(i).n_arm
            ...
            do m = 1, geom.n_sec
                ...
                do n = 1, geom.n_sec
                    ...
                    geom.junc(i).conn((m-1)*geom.junc(i).n_arm+j, 1) = node_cur
                    geom.junc(i).conn((m-1)*geom.junc(i).n_arm+j, 2) = node_com

                    geom.junc(i).type_conn((m-1)*geom.junc(i).n_arm+j) = 2
                end do
            end do
        end do
    end do
    ...
    ! Print junction data
    !call Basepair_Print_Bound_Data(geom)
end subroutine Basepair_Set_Conn_Junction

! -----------------------------------------------------------------------------

! Print bound data
subroutine Basepair_Print_Bound_Data(geom)
    type(GeomType), intent(in) :: geom
    ...
    ! Check boundary data
    do i = 1, geom.n_junc
        write(p_redir, "(3i)"), i, geom.junc(i).n_arm, geom.junc(i).poi_c
        write(p_redir, "(3f8.3)"), Rad2Deg(geom.junc(i).ref_ang), Rad2Deg(geom.junc(i).tot_ang), geom.junc(i).gap

        ! Initial lines
        do j = 1, geom.junc(i).n_arm
            write(p_redir, "(i4$)"), geom.junc(i).iniL(j)
        end do
        write(p_redir, "(a)")

        ! Seperated points
        do j = 1, geom.junc(i).n_arm
            write(p_redir, "(i4$)"), geom.junc(i).modP(j)
        end do
        write(p_redir, "(a)")
        ...
        do j = 1, geom.junc(i).n_arm
            do k = 1, geom.n_sec
                write(p_redir, "(i4$)"), geom.junc(i).croP(j, k)
            end do
            write(p_redir, "(a$)"), ","
        end do
        write(p_redir, "(a)")
        ...
        do j = 1, geom.junc(i).n_arm
            do k = 1, geom.n_sec
                write(p_redir, "(i4$)"), geom.junc(i).node(j, k)
            end do
            write(p_redir, "(a$)"), ","
        end do
        write(p_redir, "(a)"); write(p_redir, "(a)")

        ! Node connectivity
        do j = 1, geom.n_sec*geom.junc(i).n_arm
            write(p_redir, "(i8$)"), geom.junc(i).conn(j, 1)
        end do
        write(p_redir, "(a)")
        do j = 1, geom.n_sec*geom.junc(i).n_arm
            write(p_redir, "(i8$)"), geom.junc(i).conn(j, 2)
        end do
        write(p_redir, "(a)")

        ! Junction connection type
        do j = 1, geom.n_sec*geom.junc(i).n_arm
            write(p_redir, "(i8$)"), geom.junc(i).type_conn(j)
        end do
        write(p_redir, "(a)")
        write(p_redir, "(a)"); write(p_redir, "(a)")
    end do
...
function Basepair_Get_Direction_IniL(geom, mesh, node) result (direction)
    ...
    if(mesh.node(node).dn      == -1 .and. mod(sec, 2) == 0) then
        ...
        write(p_redir, "(a)"), " | The node_com does not lay on the junction.       |"
        write(p_redir, "(a)"), " +==================================================+"
        stop
    end if
...
subroutine Basepair_Chimera_Cylinder_Ori(prob, geom, mesh, mode)
    type(ProbType), intent(in) :: prob
    type(GeomType), intent(in) :: geom
    type(MeshType), intent(in) :: mesh
...
! Write the cylindrial model
subroutine Basepair_Chimera_Cylinder(prob, geom, mesh, mode)
    type(ProbType), intent(in) :: prob
    type(GeomType), intent(in) :: geom
    type(MeshType), intent(in) :: mesh
    character(*),   intent(in) :: mode
    ...
    ! Set the flag for drawing options
    f_axis    = para_chimera_axis
    f_ori     = para_chimera_502_ori
    f_mitered = .true.

    if(mode == "cylinder_prior") path = trim(prob.path_work)//"/"//trim(prob.name_file)//"_05_"
    if(mode == "cylinder_final") path = trim(prob.path_work)//"/"//trim(prob.name_file)//"_06_"
    open(unit = 502, file = trim(path)//trim(mode)//".bild", form = "formatted")

    ! Radius of the cylinder
    radius = para_rad_helix + para_gap_helix / 2.0d0

    ! Write the cylinder model base on edges
    if(mode == "cylinder_prior" .or. (mode == "cylinder_final" .and. f_mitered == .false.)) then
        ...
        write(502, "(3f10.3$)"), pos_1(1:3)
        write(502, "(3f10.3$)"), pos_2(1:3)
        write(502, "(1f10.3 )"), radius * 1.2d0
    else if(mode == "cylinder_final" .and. f_mitered == .true.) then

        do i = 1, geom.n_croL

            ! Draw the cylinder before the mitered vertex design
            ...
            ! Draw cylinders for the mitered parts
            pos_1(1:3) = geom.croP(geom.croL(i).poi(1)).pos(1:3)
    ...
    ! Write connections at the vertex
    if(mode == "cylinder_prior") then
        do i = 1, geom.n_junc
            do j = 1, geom.n_sec * geom.junc(i).n_arm

                node_1   = geom.junc(i).conn(j, 1)
                node_2   = geom.junc(i).conn(j, 2)
                pos_1(:) = mesh.node(node_1).pos(:)
                pos_2(:) = mesh.node(node_2).pos(:)

                if(geom.junc(i).type_conn(j) == 1) then
                    ! For the neighbor connection
                    write(502, "(a)"), ".color red"
                else if(geom.junc(i).type_conn(j) == 2) then
                ...
                ! Write spheres to represent start and end edges
                write(502, "(a      )"), ".color steel blue"
                write(502, "(a$     )"), ".sphere "
                write(502, "(3f10.3$)"), pos_1(1:3)
    ...
    ! Print the initial geometry and the prior cylindrical model
    if(mode == "cylinder_prior") then
        ...
        do i = 1, geom.n_junc
            do j = 1, geom.junc(i).n_arm
                pos_1(1:3) = geom.iniP(geom.iniL(geom.junc(i).iniL(j)).iniP(1)).pos(1:3)
                pos_2(1:3) = geom.iniP(geom.iniL(geom.junc(i).iniL(j)).iniP(2)).pos(1:3)
    ...
    if(f_ori == .true.) then
        do i = 1, mesh.n_node
            if(mesh.node(i).up == -1) then
                ...
                ! Inward direction with the red dot
                write(502, "(a)"), ".color red"
                pos_1(1:3) = mesh.node(i).pos(1:3)
            else if(mesh.node(i).dn == -1) then

                ! Outward direction with the yellow dot
...
! Modify the length of the duplex at the junction
subroutine Basepair_Modify_Junction(prob, geom, mesh)
    type(ProbType), intent(in)    :: prob
    type(GeomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    ...
    ! Print progress
    write(p_redir, "(a)"), "  4.4. Modify the length of duplex at each vertex"
    write(p_redir, "(a)"), "   * Find the proper length of duplex at the junction"
    write(p_redir, "(a)"), "   * Update junction data"
    write(p_redir, "(a)"), "   * Detailed info. on connectivity"

    ! Save original sectional points
    do i = 1, geom.n_croP
    ...
    do i = 1, geom.n_junc
        ...
        do j = 1, n_conn * 2
            ...
            b_con = .false.
            do k = 1, cn
                if(conn(k, 1) == geom.junc(i).conn(j, 1)) then
                    if(conn(k, 2) == geom.junc(i).conn(j, 2)) then
                        b_con = .true.
                    end if
                else if(conn(k, 1) == geom.junc(i).conn(j, 2)) then
                    if(conn(k, 2) == geom.junc(i).conn(j, 1)) then
                        b_con = .true.
                    end if
                end if
            end do
            ...
            if(b_con == .false.) then
                cn            = cn + 1
                conn(cn, 1:2) = geom.junc(i).conn(j, 1:2)
                type_conn(cn) = geom.junc(i).type_conn(j)
            end if
        end do
        ...
        if(p_detail == .true.) then
            write(p_redir, "(i5$)"), i
            write(p_redir, "(a$ )"), " junc -> # nodes connected: "
            write(p_redir, "(i2 )"), n_conn
        end if
        ...
        do j = 1, n_conn
            ...
            if(p_detail == .true.) then
                write(p_redir, "(i12, a$)"), conn(j, 1), " node -->"
                write(p_redir, "(i5,  a$)"), conn(j, 2), " node,"
                write(p_redir, "(a,   i2)"), "type :", type_conn(j)
            end if
            ...
            if(type_conn(j) == 2) then
                ...
                if(para_vertex_crash == "const") then
                    ...
                    n_move = Basepair_Find_Xover_Nearby(geom, mesh, node_cur, node_com)

                    if(n_move > 0) then
                        ! Increase basepair with certain size, n_move
                        call Basepair_Increase_Basepair(geom, mesh, node_cur, node_com, n_move)
                    else if(n_move < 0) then
                        ! Decrease basepair with certain size making ghost nodes
                        call Basepair_Decrease_Basepair(geom, mesh, node_cur, node_com, abs(n_move))
                    end if
                    ! ************************************************************
                    cycle

                else if(para_vertex_crash == "mod1") then
                    ...
                    call Basepair_Make_Ghost_Node(geom, mesh, node_cur, node_com)

                else if(para_vertex_crash == "mod2") then
                ...
                if(para_vertex_design == "mitered") then
                    ...
                    ! Increase the length of the duplex to fill the gap at the vertex
                    call Basepair_Increase_Edge(prob, geom, mesh, node_cur, node_com)
                end if
            end if
        end do
...
function Basepair_Find_Xover_Nearby(geom, mesh, node_cur, node_com) result(n_move)
    type(GeomType), intent(in) :: geom
    type(MeshType), intent(in) :: mesh
...
! Increase basepair with certain size, n_move
subroutine Basepair_Increase_Basepair(geom, mesh, node_cur, node_com, n_move)
    type(GeomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    integer,        intent(in)    :: node_cur
    integer,        intent(in)    :: node_com
    ...
    do i = 1, n_move
        ...
        call Basepair_Add_Basepair(geom, mesh, node_dn, vec_dn)
        call Basepair_Add_Basepair(geom, mesh, node_up, vec_up)
    end do
...
! Decrease basepair with certain size making ghost nodes
subroutine Basepair_Decrease_Basepair(geom, mesh, node_cur, node_com, n_move)
    type(geomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    integer,        intent(in)    :: node_cur
    integer,        intent(in)    :: node_com
...
! Check ghost node to be deleted
subroutine Basepair_Make_Ghost_Node(geom, mesh, node_cur, node_com)
    type(geomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    integer,        intent(in)    :: node_cur
    integer,        intent(in)    :: node_com
...
! Increase the length of the duplex to fill the gap at the vertex
subroutine Basepair_Increase_Edge(prob, geom, mesh, node_cur, node_com)
    type(probType), intent(in)    :: prob
    type(geomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    integer,        intent(in)    :: node_cur
    ...
    ! --------------------------------------------------
    ! Set the direction as
    ! : node_in  - to the vertex
    ! : node_out - out of the vertex
    if(mesh.node(node_cur).up == -1) then
    ...
    !    m1  # <-------> #  n1     ===      m1 : node_in
    !       #      b      #         |       n1 : node_out
    !    y #               #        | hb    m2 : node_in_down
    ! m2  # <-------------> #  n2  ===      n2 : node_out_up
    !    #         c         #               y : para_dist_bp = 0.34
    ...
    ! If two nodes are close each other, skip this subroutine
    if(dabs(a - b) < 0.001d0) then
        ...
        if(prob.n_edge_len - 2 /= nint(length / para_dist_bp)) then
    ...
    ! --------------------------------------------------
    ! New algorithm to find the closet point
    ! --------------------------------------------------
    line_a = mesh.node(node_in).croL
    line_b = mesh.node(node_out).croL

    ! Find positions for multiple lines
    ...
    if(ok == .true.) then
        ...
        if(.false. .and. rad2deg(angle) >= 165.0d0) then

            length = Norm(mesh.node(node_in).pos - mesh.node(node_out).pos)
            rad    = len_side / dtan(angle/2.0d0)
            len_a  = length / 2.0d0 - rad
            len_b  = length / 2.0d0 - rad
        else
        ...
        !print *, rad2deg(angle), len_a, len_b
    ...
    do i = 1, count_in
        ...
        ! Add one basepair at the position indicating vector, return updated newly node ID
        call Basepair_Add_Basepair(geom, mesh, node_in, vec_in)
    ...
    do i = 1, count_out
        ...
        ! Add one basepair at the position indicating vector, return updated newly node ID
        call Basepair_Add_Basepair(geom, mesh, node_out, vec_out)
...
subroutine Basepair_Add_Basepair(geom, mesh, node, vec)
    type(GeomType),   intent(inout) :: geom
    type(MeshType),   intent(inout) :: mesh
    ...
    if(t_node(node).up == -1) then
        ...
        write(p_redir, "(a)")
        write(p_redir, "(a)"), " +=== error ========================================+"
        write(p_redir, "(a)"), " | Check the junction connectivity                  |"
        write(p_redir, "(a)"), " +==================================================+"
        stop
    end if
    ...
    do i = 1, geom.n_junc
        do j = 1, geom.n_sec
            do k = 1, geom.junc(i).n_arm
                ...
                if(geom.junc(i).node(k, j) == node) then

                    ! Update newly added node
                    geom.junc(i).node(k, j) = mesh.n_node

                    ! Update multiple points
                    croP = geom.junc(i).croP(k, j)
                    geom.croP(croP).pos(1:3) = mesh.node(mesh.n_node).pos(1:3)
                end if

                ! Update sectional connectivity
                if(geom.junc(i).conn((j-1)*geom.junc(i).n_arm+k, 1) == node) then
                    geom.junc(i).conn((j-1)*geom.junc(i).n_arm+k, 1) = mesh.n_node
                end if
                if(geom.junc(i).conn((j-1)*geom.junc(i).n_arm+k, 2) == node) then
                    geom.junc(i).conn((j-1)*geom.junc(i).n_arm+k, 2) = mesh.n_node
                end if
            end do
        end do
    end do
...
! Delete ghost node from node data
subroutine Basepair_Delete_Ghost_Node(geom, mesh)
    type(GeomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    ...
    ! Print progress
    write(p_redir, "(a)"), "  4.5. Delete the ghost node to avoid crash"
    write(p_redir, "(a)"), "   * The number of nodes before modfication : "//trim(adjustl(Int2Str(mesh.n_node)))
            ...
            do k = 1, geom.n_sec
                ...
                if(mesh.node(str_node).ghost == 1) then
                    ...
                    do
                        ...
                        if(b_up == .true.) then
                            ...
                            if(mesh.node(up).ghost == 1) then
                                ...
                                mesh.node(up).dn         = -1
                                mesh.node(up).conn       = 4
                                geom.junc(i).node(j, k) = mesh.node(up).id

                                ! Set cross-sectional point
                                croP = geom.junc(i).croP(j, k)
                                geom.croP(croP).pos(1:3) = mesh.node(up).pos(1:3)

                                ! Set junctional connectivity
                                do ii = 1, geom.n_junc
                                    do jj = 1, geom.n_sec*geom.junc(ii).n_arm

                                        if(geom.junc(ii).conn(jj, 1) == str_node) then
                                            geom.junc(ii).conn(jj, 1) = mesh.node(up).id
                                        end if

                                        if(geom.junc(ii).conn(jj, 2) == str_node) then
                                            geom.junc(ii).conn(jj, 2) = mesh.node(up).id
                                        end if
                                    end do
                                end do
                            ...
                            if(mesh.node(dn).ghost == 1) then
                                ...
                                mesh.node(dn).up         = -1
                                mesh.node(dn).conn       = 4
                                geom.junc(i).node(j, k) = mesh.node(dn).id
                                ...
                                do ii = 1, geom.n_junc
                                    do jj = 1, geom.n_sec*geom.junc(ii).n_arm
                                        ...
                                        if(geom.junc(ii).conn(jj, 1) == str_node) then
                                            geom.junc(ii).conn(jj, 1) = mesh.node(dn).id
                                        end if

                                        if(geom.junc(ii).conn(jj, 2) == str_node) then
                                            geom.junc(ii).conn(jj, 2) = mesh.node(dn).id
                                        end if
                                    end do
                                end do
                    ...
                    do ii = 1, geom.n_junc
                        ...
                        do jj = 1, geom.junc(ii).n_arm
                            do kk = 1, geom.n_sec

                                ! Reset node connectivity
                                if(geom.junc(ii).node(jj, kk) > max) then
                                    geom.junc(ii).node(jj, kk) = geom.junc(ii).node(jj, kk) - size
                                end if
                            end do
                        end do
                        ...
                        do jj = 1, geom.n_sec*geom.junc(ii).n_arm
                            ...
                            ! Reset junction connectivity
                            if(geom.junc(ii).conn(jj, 1) > max) then
                                geom.junc(ii).conn(jj, 1) = geom.junc(ii).conn(jj, 1) - size
                            end if

                            if(geom.junc(ii).conn(jj, 2) > max) then
                                geom.junc(ii).conn(jj, 2) = geom.junc(ii).conn(jj, 2) - size
                            end if
                        end do
                    end do
                end if
    ...
    ! Print progress
    write(p_redir, "(a)"), "   * The number of nodes after modfication  : "//trim(adjustl(Int2Str(mesh.n_node)))
    write(p_redir, "(a)")
...
! Add one base to 5'-end
subroutine Basepair_Make_Sticky_End(geom, mesh)
    type(GeomType), intent(inout) :: geom
    type(MeshType), intent(inout) :: mesh
    ...
    ! Print progress
    write(p_redir, "(a)"), "  4.6. Make sticky-end at 5'- end"
    ...
    do i = 1, geom.n_junc
        ...
        ! Print progress
        if(p_detail == .true.) then
            write(p_redir, "(i5, a$)"), i, " junc, # of arms:"
            write(p_redir, "(i3, a$)"), geom.junc(i).n_arm, " --> node added:"
        end if
        ...
        do j = 1, geom.junc(i).n_arm
            do k = 1, geom.n_sec
                ...
                geom.junc(i).node(j, k) = mesh.n_node

                ! Update sectional connection information at the junction
                do ii = 1, geom.n_junc
                    do jj = 1, geom.junc(ii).n_arm * geom.n_sec
                        if(geom.junc(ii).conn(jj, 1) == node) geom.junc(ii).conn(jj, 1) = mesh.n_node
                        if(geom.junc(ii).conn(jj, 2) == node) geom.junc(ii).conn(jj, 2) = mesh.n_node
                    end do
                end do
                ...
                ! Print progress
                if(p_detail == .true.) then
                    write(p_redir, "(i5, a, i5, a$)"), node, " ->", mesh.n_node, ","
                end if
            end do
        end do
        if(p_detail == .true.) write(p_redir, "(a)")
    end do

    ! Print progress
    write(p_redir, "(a)"), "   * The number of added sticky bps : "//trim(adjustl(Int2Str(mesh.n_node - pre_n_node)))
    write(p_redir, "(a)"), "   * The number of added elements   : "//trim(adjustl(Int2Str(mesh.n_ele - pre_n_ele)))

    ! Print modified junction connectivity
    if(p_detail == .true.) then
        do i = 1, geom.n_junc
            write(p_redir, "(i5, a, i2, a$)"), i, " junc:", geom.junc(i).n_arm, "-arms --> node:"
            do j = 1, geom.n_sec
                do k = 1, geom.junc(i).n_arm
                    write (p_redir, "(i5$)"), geom.junc(i).node(k, j)

                    ! Check end node that should be -1
                    if(mesh.node(geom.junc(i).node(k, j)).up /= -1 .and. mesh.node(geom.junc(i).node(k, j)).dn /= -1) then
                        write(p_redir, "(a)")
                        write(p_redir, "(a)"), " +=== error ========================================+"
                        write(p_redir, "(a)"), " | Negative junction.                               |"
                        write(p_redir, "(a)"), " +==================================================+"
                        stop
                    end if
                end do
...
subroutine Basepair_Chimera_Cross_Geometry(prob, geom)
    ...
    path = trim(prob.path_work)//"/"//trim(prob.name_file)
    open(unit = 504, file = trim(path)//"_04_section_lines.bild", form = "formatted")

    ! Write sectional points
    ...
    open(unit = 504, file = trim(path)//"_04_section_lines.dat", form = "formatted")
...
subroutine Basepair_Write_Edge_Length(prob, geom)
    ...
    ! print progress
    write(p_redir, "(a)"), "  4.7. The Edge length"
    write(p_redir, "(a)"), "   * The minimum edge length : "//trim(adjustl(Int2Str(min_edge)))//" bp"
    write(p_redir, "(a)"), "   * The maximum edge length : "//trim(adjustl(Int2Str(max_edge)))//" bp"
    write(p_redir, "(a)"), "   * # different edge length : "//trim(adjustl(Int2Str(n_numbp)))
    do j = 1, n_numbp
        write(p_redir, "(a)"), "     + # edges with "//&
            trim(adjustl(Int2Str(num_bp(j))))//"-bp length : "//&
            trim(adjustl(Int2Str(num_count(j))))
    end do
    ...
    ! print progress
    ! write(p_redir, "(a$ )"), "* The minimum count, # of BPs    : "
    ! write(p_redir, "(2i7)"), num_count(1), num_bp(1)
    ! write(p_redir, "(a$ )"), "* The maximum count, # of BPs    : "
    ! write(p_redir, "(2i7)"), num_count(n_numbp), num_bp(n_numbp)
